...
// ── 점근선을 정수 자리에서만 짚지 않는다 ────────────────────
test('주기적으로 튀는 함수에 수평 점근선을 붙이지 않는다', () => {
  // mod(10⁵, 2) = 0, mod(10⁷, 2) = 0 … 짝수만 짚으면 늘 같은 값이라 속는다
  assert.equal(has(fa((x) => ((x % 2) + 2) % 2), '수평 점근선'), false);
  // 진짜 수평 점근선은 찾는다
  assert.equal(has(fa((x) => 1 / (x * x - 1)), '수평 점근선'), true);
  assert.equal(has(fa((x) => (2 * x + 1) / (x - 1)), '수평 점근선'), true);
});

// ── 모형 고르기 ────────────────────────────────────────
test('가려낼 수 없을 만큼 비슷하면 단순한 모형을 고른다', () => {
  // 잔차를 9% 줄인 것은 "직선"을 "거듭제곱"으로 바꿔 말할 근거가 못 된다
  const r = fitModels([1, 2, 3, 4, 5], [2.1, 3.9, 6.2, 8.1, 9.8], { maxDegree: 3 });
  assert.equal(r[0].name, '일차(선형)');
});

test('그래도 진짜 다른 모형은 가려낸다', () => {
  assert.equal(fitModels([1, 2, 3, 4, 5], [2, 4, 8, 16, 32], { maxDegree: 3 })[0].name, '지수');
  assert.equal(fitModels([1, 2, 3, 4, 5], [1, 4, 9, 16, 25], { maxDegree: 3 })[0].name, '이차');
  assert.equal(
    fitModels([1, 2, 3, 4, 5, 6], [1, 1.41, 1.73, 2, 2.24, 2.45], { maxDegree: 3 })[0].name,
    '거듭제곱',
  );
});
