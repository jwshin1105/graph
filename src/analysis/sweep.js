...
/** 훑을 때 쓰는 성긴 계산 설정 — 위상(가지 수)만 세면 되므로 해상도를 낮춘다 */
const FAST = { coarsePx: 22, refine: 6 };

/**
 * 객체 하나의 이산 특징.
 * @returns {{key:string, label:string}|null}
 */
export function objectSignature(obj, bounds, compute) {
  let d;
  try {
    d = compute(obj, bounds, FAST);
  } catch {
    return null;
  }
  if (!d) return null;
...
/** 여러 객체의 서명을 하나로 묶는다 */
export function combinedSignature(objects, bounds, compute) {
  const parts = [];
  const labels = [];
  for (const o of objects) {
    const s = objectSignature(o, bounds, compute);
    if (!s) continue;
...
export function* sweepSteps(cfg) {
  const { objects, setParam, min, max, bounds, compute, samples = 41 } = cfg;
  const span = max - min;
  if (!(span > 0) || !objects.length) return { stages: [], transitions: [], events: [], samples: [] };

  let done = 0;
  const total = samples + 40;
  const sigAt = (t) => {
    setParam(t);
    done++;
  ...
  for (let i = 0; i < samples; i++) {
    ...
    yield Math.min(0.95, done / total);
  }
  ...
  for (let i = 1; i < pts.length; i++) {
    ...
    for (let k = 0; k < 34 && hi - lo > span * 1e-9; k++) {
      const mid = (lo + hi) / 2;
      const sg = sigAt(mid);
      if (!sg) break;
      if (sg.key === loSig.key) { lo = mid; loSig = sg; }
      else { hi = mid; hiSig = sg; }
      if (k % 6 === 5) yield Math.min(0.95, done / total);
    }
    const at = (lo + hi) / 2;
  ...
  for (const st of stages) {
    ...
    if (st.to - st.from < narrow && prev && st.sig && prev.sig && st.sig.key !== prev.sig.key) {
      events.push({ at: snapNice(st.from, st.to, span), sig: st.sig, band: [st.from, st.to] });
      prev.to = st.to;
      continue;
    }
    kept.push({ ...st });
  ...
  for (const st of stages) {
    const tally = new Map();
    for (const f of [0.17, 0.34, 0.5, 0.66, 0.83]) {
  ...
  for (let i = 1; i < stages.length; i++) {
    ...
    const at = ev ? ev.at : snapNice(b.from, b.from, span);
    transitions.push({
      at, before: a.sig, after: b.sig,
      isolated: ev ? ev.sig : (near ? near.isolated : null),
      raw: b.from,
      atText: pretty(at),
      approx: Math.abs(at - b.from) > span * 1e-7,
    });
  }
  ...
  return { stages, transitions, events: soloEvents, samples: pts };
}
...
function snapNice(lo, hi, span = 0) {
  ...
  for (const q of denoms) {
    ...
    if (inRange(k / q)) return k === 0 ? 0 : k / q;
  }
  for (const base of [Math.PI, Math.E, Math.SQRT2]) {
    for (const q of [1, 2, 3, 4]) {
      const k = Math.round(((lo + hi) / 2) / (base / q));
      if (Math.abs(k) <= 12 && inRange((k * base) / q)) return (k * base) / q;
    }
  }
  const mid = (lo + hi) / 2;
  const step = Math.pow(10, Math.floor(Math.log10(Math.max(hi - lo, 1e-12))));
  const r = Math.round(mid / step) * step;
  const out = inRange(r) ? r : mid;
  return out === 0 ? 0 : out;          // -0 을 0 으로
}
